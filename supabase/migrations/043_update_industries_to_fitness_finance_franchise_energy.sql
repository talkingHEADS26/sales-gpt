-- Align supported industries to: fitness, finance, franchise, energy.
-- Includes data mapping for legacy keys and updates signup validation logic.

update public.organizations
set industry_key = case industry_key
  when 'automotive' then 'franchise'
  when 'insurance' then 'finance'
  when 'physio' then 'fitness'
  else industry_key
end,
prompt_profile_key = case prompt_profile_key
  when 'automotive' then 'franchise'
  when 'insurance' then 'finance'
  when 'physio' then 'fitness'
  else prompt_profile_key
end
where industry_key in ('automotive', 'insurance', 'physio')
   or prompt_profile_key in ('automotive', 'insurance', 'physio');

update public.full_sales_avatar_snapshots
set industry_key = case industry_key
  when 'automotive' then 'franchise'
  when 'insurance' then 'finance'
  when 'physio' then 'fitness'
...
update public.complaint_management_avatar_snapshots
set industry_key = case industry_key
  when 'automotive' then 'franchise'
  when 'insurance' then 'finance'
  when 'physio' then 'fitness'
...
update public.appointment_setting_avatar_snapshots
set industry_key = case industry_key
  when 'automotive' then 'franchise'
  when 'insurance' then 'finance'
  when 'physio' then 'fitness'
...
alter table public.full_sales_avatar_snapshots
  drop constraint if exists full_sales_avatar_snapshots_industry_key_check,
  add constraint full_sales_avatar_snapshots_industry_key_check
    check (industry_key in ('fitness', 'finance', 'franchise', 'energy'));

alter table public.complaint_management_avatar_snapshots
  drop constraint if exists complaint_management_avatar_snapshots_industry_key_check,
  add constraint complaint_management_avatar_snapshots_industry_key_check
    check (industry_key in ('fitness', 'finance', 'franchise', 'energy'));
